...
images,labels_vec,images_valid,labels_valid_vec = np.asarray(Imagelist_train)/255.0,trainonehot,np.asarray(Imagelist_test)/255.0,testonehot
...
h_fc2 = tf.nn.relu6(tf.matmul(h_fc1_drop, W_fc2) + b_fc2)
...
for i in range(100000):
    ...
    if((i%5000)==0):
        print("accuracy   @  ",i,"  :  ",accuracy.eval({x:images_valid, y_:test_label, keep_prob:1.0}))
        print()
        print("error   @  ",i,"  :  ",cross_entropy.eval({x:images_valid, y_:test_label, keep_prob:1.0}))
        print()
#        print("chk1   :  ",)
